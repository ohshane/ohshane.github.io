...
Since $D$ is trained with negative BCE loss,
$\theta_\text{d}$ is updated with gradient ascent,
while $\theta_\text{g}$ is updated with gradient descent.
...
As a result, $D$ converges to $D^{*}(\mathbf x) = \frac{ p_\text{data}(\mathbf x) }{ p_\text{data}(\mathbf x) + p_\text{g}(\mathbf x) }$.
As you can see in (d) of @fig-training-gans, there is a point where we reach a point
which both $D$ and $G$ cannot improve because $D(\mathbf x) = \frac{1}{2}$.

This is also called as the global optimum of GANs.

### Proof of the Global Optimality

The min-max game ends