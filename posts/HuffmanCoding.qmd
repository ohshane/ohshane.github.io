...
Similar to max-heap, we build a Huffman tree using the frequency.
...
class HuffmanTree:
    ...
    def build(self):
        ...
        while len(nodes) > 1:
            node1 = nodes.pop(nodes.index(min(nodes)))
...
tree = HuffmanTree(Counter("AN APPLE A DAY KEEPS THE DOCTOR AWAY"))
```
...
show(s)
```
You might be familiar with the pangram `THE QUICK BROWN FOX JUMPS OVER THE LAZY DOG`.
...
Just for fun, let's work with the images too.
...
plt.title(f"compression rate: {(1 - (bits/8) / len(s))*100:.1f}%")
plt.axis('off')
plt.show()
